...
const char* FalconClient::URL = "http://127.0.0.1:1988/v1/push";

FalconClient::FalconClient(const std::string& url)
  : url_(url) {
...
bool FalconClient::send(const std::map<std::string, int>& value) {
  if (value.empty()) {
    return false;
  }
...
bool FalconClient::post(const std::string& data) {
