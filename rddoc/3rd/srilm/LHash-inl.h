...
KeyT LHash<KeyT, DataT>::zeroKey = KeyT();
...
#define BODY_SIZE(b, n) (sizeof(*BODY(b)) + ((n)-1) * sizeof(BODY(b)->data[0]))

/*
 * Compute the actual minimum size required for a given number of entries
 */
static inline unsigned roundSize(unsigned size) {
...
template <class KeyT, class DataT>
void LHash<KeyT, DataT>::alloc(unsigned size) {
  ...
  body = (LHashBody<KeyT, DataT> *)malloc(BODY_SIZE(body, maxEntries));
  assert(body != 0);
...
template <class KeyT, class DataT>
void LHash<KeyT, DataT>::clear(unsigned size) {
  if (body) {
...
template <class KeyT, class DataT>
LHash<KeyT, DataT> &LHash<KeyT, DataT>::operator=(
    const LHash<KeyT, DataT> &other) {
  if (&other == this) {
    return *this;
  }
 ...
 */
template <class KeyT, class DataT>
bool LHash<KeyT, DataT>::locate(KeyT key, unsigned &index) const {
  assert(!Map_noKeyP(key));

  if (body) {
    unsigned maxBits = BODY(body)->maxBits;
    MapEntry<KeyT, DataT> *data = BODY(body)->data;
...
template <class KeyT, class DataT>
DataT *LHash<KeyT, DataT>::find(KeyT key, bool &foundP) const {
  unsigned index;
...
template <class KeyT, class DataT>
KeyT LHash<KeyT, DataT>::getInternalKey(KeyT key, bool &foundP) const {
  unsigned index;
...
template <class KeyT, class DataT>
DataT *LHash<KeyT, DataT>::insert(KeyT key, bool &foundP) {
  unsigned index;
...
template <class KeyT, class DataT>
bool LHash<KeyT, DataT>::remove(KeyT key, DataT *removedData) {
  unsigned index;
  ...
  if ((locate(key, index))) {
    ...
    if (BODY(body)->maxBits < minHashBits) {
      ...
      memmove(&BODY(body)->data[index], &BODY(body)->data[index + 1],
      ...
      while (1) {
        ...
        if (!locate(BODY(body)->data[index].key, newIndex)) {
          // cerr << "MOVING key = " << (BODY(body)->data[index].key) << " from
          // " << index << " to " << newIndex << endl;
          memcpy(&(BODY(body)->data[newIndex]), &(BODY(body)->data[index]),
...
template <class KeyT, class DataT>
void LHashIter<KeyT, DataT>::sortKeys() {
  /*
  ...
  if (sortFunction != 0) {
    std::sort(sortedIndex, sortedIndex + numEntries, *this);
  }
...
LHashIter<KeyT, DataT>::LHashIter(const LHash<KeyT, DataT> &lhash,
    ...
    : myLHashBody(BODY(lhash.body)),
      current(0),
      numEntries(lhash.numEntries()),
      sortFunction(keyCompare),
      sortedKeys(0) {
  /*
...
LHashIter<KeyT, DataT>::LHashIter(const LHashIter<KeyT, DataT> &iter)
    : myLHashBody(iter.myLHashBody),
      current(iter.current),
      numEntries(iter.numEntries),
      sortFunction(iter.sortFunction),
      sortedKeys(0) {
  if (iter.sortedKeys) {
    sortedKeys = new KeyT[numEntries];
    assert(sortedKeys != 0);
 ...
 */
template <class KeyT, class DataT>
bool LHashIter<KeyT, DataT>::operator()(const unsigned idx1,
                                        const unsigned idx2) {
  return (*(compFnType)sortFunction)(BODY(myLHashBody)->data[idx1].key,
...
template <class KeyT, class DataT>
void LHashIter<KeyT, DataT>::init() {
  delete[] sortedKeys;
  sortedKeys = 0;
...
template <class KeyT, class DataT>
DataT *LHashIter<KeyT, DataT>::next(KeyT &key) {
  if (myLHashBody == 0) {
    return 0;
  } else {
    unsigned index;
