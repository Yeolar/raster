/*
 * TLSWrapper.h -- A template that wraps a thread-local storage variable.
 *
 * If NO_TLS is defined then the macros in this file will simply create
 * static variables, thereby producing a single-threaded build.
 */
...
#define TLSW(type, name) ::srilm::TLSWrapper<type> name(1, false)
// Declare and define a non-POD TLS variable (call its constructor)
#define TLSWC(type, name) ::srilm::TLSWrapper<type> name(1, true)
// Declare and define a TLS array
#define TLSW_ARRAY(type, name, size) ::srilm::TLSWrapper<type> name(size)

// Declare a TLS variable of a given type
#define TLSW_DECL(type, name) ::srilm::TLSWrapper<type> name
// Declare a TLS array of a given type
#define TLSW_DECL_ARRAY(type, name, size) ::srilm::TLSWrapper<type> name

// Define a previously declared TLS variable
#define TLSW_DEF(type, name) \
  ::srilm::TLSWrapper<type> name = ::srilm::TLSWrapper<type>(1, false)
// Define a previously declared non-POD TLS variable
#define TLSW_DEFC(type, name) \
  ::srilm::TLSWrapper<type> name = ::srilm::TLSWrapper<type>(1, true)
// Define a previously declared array TLS variable
#define TLSW_DEF_ARRAY(type, name, size) \
  ::srilm::TLSWrapper<type> name = ::srilm::TLSWrapper<type>(size)
...
class TLSWrapper {
public:
  TLSWrapper(size_t numP = 1, bool constructP = false) {
    key = TLS_CREATE_KEY();
    num = numP;
    construct = constructP;
  }

  ~TLSWrapper() { TLS_FREE_KEY(key); }
