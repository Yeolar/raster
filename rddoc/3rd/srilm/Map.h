 ...
 *
 * KeyT getInternalKey(KeyT key, bool &foundP)
 ...
 *
 * DataT *remove(KeyT key, bool &foundP)
...
public:
  virtual DataT *find(KeyT key, bool &foundP) const = 0;
  /* key lookup -- returns a
   * zero object and sets foundP =
   * false if key is not found */
  virtual KeyT getInternalKey(KeyT key, bool &foundP) const = 0;
  /* return the internalized key
   * for an entry -- use with caution */
  virtual DataT *insert(KeyT key, bool &foundP) = 0;
  /* create or change a value */
  virtual DataT *remove(KeyT key, bool &foundP) = 0;
 ...
 */
template <class KeyT>
inline KeyT Map_copyKey(KeyT key) {
  return key;
}
template <class KeyT>
 ...
 * bool Map_noKeyP(KeyT key) checks for it.
 *
 * The template and specializations below cover the most common cases.
 * Others need to be defined by the user.
 */
template <class KeyT>
inline void Map_noKey(const KeyT *&key) {
  key = 0;
}
template <class KeyT>
inline bool Map_noKeyP(const KeyT *key) {
  return key == 0;
}
...
inline bool Map_noKeyP(int key) { return key == IntNokeyValue; }
inline void Map_noKey(short int &key) { key = ShortNokeyValue; }
inline bool Map_noKeyP(short int key) { return key == ShortNokeyValue; }
inline void Map_noKey(long int &key) { key = LongNokeyValue; }
inline bool Map_noKeyP(long int key) { return key == LongNokeyValue; }
...
inline bool Map_noKeyP(unsigned key) { return key == UIntNokeyValue; }
inline void Map_noKey(short unsigned &key) { key = UShortNokeyValue; }
inline bool Map_noKeyP(short unsigned key) { return key == UShortNokeyValue; }
inline void Map_noKey(long unsigned &key) { key = ULongNokeyValue; }
inline bool Map_noKeyP(long unsigned key) { return key == ULongNokeyValue; }

/*
 * Floats use HUGE_VAL as the non-key
 */
inline void Map_noKey(float &key) { key = HUGE_VAL; }
inline bool Map_noKeyP(float key) { return key == HUGE_VAL; }
inline void Map_noKey(double &key) { key = HUGE_VAL; }
inline bool Map_noKeyP(double key) { return key == HUGE_VAL; }
