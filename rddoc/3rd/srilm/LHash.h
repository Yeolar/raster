/*
 * LHash.h -- Hash table with linear search.
 *
...
class LHash;  // forward declaration
template <class KeyT, class DataT>
class LHashIter;  // forward declaration
...
template <class KeyT>
inline bool LHash_equalKey(KeyT key1, KeyT key2) {
  return (key1 == key2);
}

inline bool LHash_equalKey(const char *key1, const char *key2) {
 ...
 */
inline size_t LHash_hashKey(unsigned long key, unsigned maxBits) {
  return (((key * 1103515245 + 12345) >> (30 - maxBits)) & hashMask(maxBits));
}

inline size_t LHash_hashKey(const char *key, unsigned maxBits) {
  /*
