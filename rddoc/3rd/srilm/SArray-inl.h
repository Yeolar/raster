...
KeyT SArray<KeyT, DataT>::zeroKey;
...
void SArray<KeyT, DataT>::alloc(unsigned size) {
  body = (SArrayBody<KeyT, DataT> *)malloc(BODY_SIZE(body, size));
  assert(body != 0);
...
template <class KeyT, class DataT>
void SArray<KeyT, DataT>::setsize(unsigned size) {
  if (body == 0 && size != 0) {
    alloc(size);
  }
}
...
template <class KeyT, class DataT>
SArray<KeyT, DataT> &SArray<KeyT, DataT>::operator=(
    const SArray<KeyT, DataT> &other) {
  if (&other == this) {
    return *this;
  }
 ...
 */
template <class KeyT, class DataT>
bool SArray<KeyT, DataT>::locate(KeyT key, unsigned &index) const {
...
template <class KeyT, class DataT>
DataT *SArray<KeyT, DataT>::find(KeyT key, bool &foundP) const {
  unsigned index;
...
template <class KeyT, class DataT>
KeyT SArray<KeyT, DataT>::getInternalKey(KeyT key, bool &foundP) const {
  unsigned index;
...
template <class KeyT, class DataT>
DataT *SArray<KeyT, DataT>::insert(KeyT key, bool &foundP) {
  unsigned index;
  ...
  if ((foundP = locate(key, index))) {
    ...
    if (nEntries == maxEntries) {
      ...
      void *newBody =
          (SArrayBody<KeyT, DataT> *)malloc(BODY_SIZE(body, newMaxEntries));
      assert(newBody != 0);
       ...
       */
      memcpy(&(BODY(newBody)->data[0]), &(BODY(body)->data[0]),
             index * sizeof(BODY(body)->data[0]));
      memcpy(&(BODY(newBody)->data[index + 1]), &(BODY(body)->data[index]),
       ...
       */
      memmove(&(BODY(body)->data[index + 1]), &(BODY(body)->data[index]),
...
template <class KeyT, class DataT>
bool SArray<KeyT, DataT>::remove(KeyT key, DataT *removedData) {
  unsigned index;
...
SArrayIter<KeyT, DataT>::SArrayIter(const SArray<KeyT, DataT> &sarray,
    ...
    : mySArrayBody(BODY(sarray.body)),
      current(0),
      numEntries(sarray.numEntries()),
...
SArrayIter<KeyT, DataT>::SArrayIter(const SArrayIter<KeyT, DataT> &iter)
    : mySArrayBody(iter.mySArrayBody),
 ...
 */
template <class KeyT, class DataT>
bool SArrayIter<KeyT, DataT>::operator()(const unsigned idx1,
                                         const unsigned idx2) {
  return (*(compFnType)sortFunction)(BODY(mySArrayBody)->data[idx1].key,
...
template <class KeyT, class DataT>
void SArrayIter<KeyT, DataT>::init() {
  delete[] sortedKeys;
  sortedKeys = 0;
