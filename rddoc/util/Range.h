...
#include "rddoc/util/SpookyHashV2.h"
...
public:
  ...
  size_type rfind(value_type c) const {
    return rdd::rfind(castToConst(), c);
  }
   ...
   * Example:
   *
   *  rdd::StringPiece s("sample string for split_next");
   *  auto p = s.split_step(' ');
   *
   ...
   * Example:
   *
   *  void do_some_parsing(rdd::StringPiece s) {
   *    auto version = s.split_step(' ', [&](rdd::StringPiece x) {
   *      if (x.empty()) {
   ...
   *
   *  struct Foo {
   *    void parse(rdd::StringPiece s) {
   *      s.split_step(' ', parse_field, bar, 10);
   *      s.split_step('\t', parse_field, baz, 20);
   *
   *      auto const kludge = [](rdd::StringPiece x, int &out, int def) {
   *        if (x == "null") {
   *          out = 0;
   *        } else {
   ...
   *    int foo;
   *
   *    static parse_field(rdd::StringPiece s, int &out, int def) {
   *      try {
   *        out = rdd::to<int>(s);
...
template <class T>
struct hasher<rdd::Range<T*>,
              typename std::enable_if<std::is_pod<T>::value, void>::type> {
  size_t operator()(rdd::Range<T*> r) const {
