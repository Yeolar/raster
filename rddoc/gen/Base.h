...
#include "rddoc/util/Optional.h"
#include "rddoc/util/Range.h"
#include "rddoc/gen/Core.h"
...
class To {
 public:
  template <class Value>
  Dest operator()(Value&& value) const {
    return ::rdd::to<Dest>(std::forward<Value>(value));
  }
};
...
#define GENERATOR(TYPE)                            \
  ::rdd::gen::detail::GeneratorBuilder<TYPE>() + \
