...
TEST(Gen, FromIterators) {
  ...
  auto gen = from(rdd::range(source.begin() + 1, source.end() - 1));
...
TEST(Gen, FilterDefault) {
  ...
  {
    // Default filter on Optionals should remove rdd::null
    const auto expected =
...
TEST(Gen, Unwrap) {
  ...
  auto oup = rdd::make_optional(rdd::make_unique<int>(5));
  ...
  auto moved1 = std::move(oup) | unwrapOr(rdd::make_unique<int>(6));
  ...
  auto moved2 = std::move(oup) | unwrapOr(rdd::make_unique<int>(7));
  // oup's still-valid nullptr value wins here, the pointer to 7 doesn't apply
  EXPECT_FALSE(moved2);

  oup.clear();
  auto moved3 = std::move(oup) | unwrapOr(rdd::make_unique<int>(8));
  ...
  {
    auto opt = rdd::make_optional(std::make_shared<int>(8));
    auto fallback = unwrapOr(rdd::make_unique<int>(9));
