...
void EventLoop::handle(Event* event, uint32_t etype) {
  RDD_EVLOG(V2, event) << "on event";
  switch (event->type()) {
    case Event::LISTEN:
    ...
    default:
      ...
      loop_->closePeer(event);
      break;
  }
}

void EventLoop::onListen(Event* event) {
...
void EventLoop::onConnect(Event* event) {
...
void EventLoop::onRead(Event* event) {
...
void EventLoop::onWrite(Event* event) {
...
void EventLoop::onComplete(Event* event) {
...
void EventLoop::onTimeout(Event *event) {
  assert(event->type() == Event::TIMEOUT);
  RDDMON_CNT("conn.timeout-" + event->label());
  loop_->closePeer(event);
}

void EventLoop::onError(Event* event) {
