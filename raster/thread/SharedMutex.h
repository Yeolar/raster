...
//
// SharedMutex provides all of the methods of rdd::RWSpinLock,
...
// I compared it to rdd::RWSpinLock, rdd::RWTicketSpinLock64,
...
// of rdd/test/SharedMutexTest.cpp for lots of microbenchmark results.
//
// Comparison to rdd::RWSpinLock:
//
...
class SharedMutexImpl {
 ...
 private:
  typedef typename rdd::Futex Futex;
 ...
 private:
  RDD_ALIGN_TO_AVOID_FALSE_SHARING static DeferredReaderSlot deferredReaders
      [kMaxDeferredReaders *
  ...
  while (true) {
    ...
    if (canAlreadyDefer || (aboveDeferThreshold && !drainInProgress)) {
      ...
      if (slotValue != 0) {
        ...
        uint32_t bestSlot =
            (uint32_t)rdd::AccessSpreader::current(kMaxDeferredReaders);
