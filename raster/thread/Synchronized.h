 ...
 * module.  See rdd/docs/Synchronized.md for a more complete explanation of
...
#include "raster/thread/LockTraits.h"
...
class SynchronizedBase<Subclass, detail::MutexLevel::SHARED> {
 public:
  using LockedPtr = ::rdd::LockedPtr<Subclass, LockPolicyExclusive>;
  using ConstWLockedPtr =
      ::rdd::LockedPtr<const Subclass, LockPolicyExclusive>;
  using ConstLockedPtr = ::rdd::LockedPtr<const Subclass, LockPolicyShared>;
...
class SynchronizedBase<Subclass, detail::MutexLevel::UPGRADE>
 ...
 public:
  using UpgradeLockedPtr = ::rdd::LockedPtr<Subclass, LockPolicyUpgrade>;
  using ConstUpgradeLockedPtr =
      ::rdd::LockedPtr<const Subclass, LockPolicyUpgrade>;
  using UpgradeLockedGuardPtr =
      ::rdd::LockedGuardPtr<Subclass, LockPolicyUpgrade>;
  using ConstUpgradeLockedGuardPtr =
      ::rdd::LockedGuardPtr<const Subclass, LockPolicyUpgrade>;
 ...
 * rdd::SharedMutex, rdd::RWSpinLock, and rdd::SpinLock.
...
struct Synchronized : public SynchronizedBase<
 ...
 private:
  ...
  friend class rdd::LockedPtrBase;
  template <class LockedType, class LockPolicy>
  friend class rdd::LockedPtr;
  template <class LockedType, class LockPolicy>
  friend class rdd::LockedGuardPtr;
...
class LockedPtrBase {
 public:
  using MutexType = Mutex;
  friend class rdd::ScopedUnlocker<SynchronizedType, LockPolicy>;
 ...
 */
#define SYNCHRONIZED_VAR(var) RDD_CONCATENATE(SYNCHRONIZED_##var##_, __LINE__)
 ...
 *
 * Refer to rdd/docs/Synchronized.md for a detailed explanation and more
 * examples.
 */
#define SYNCHRONIZED(...)                                        \
  if (bool SYNCHRONIZED_VAR(state) = false) {                    \
    ...
    for (auto SYNCHRONIZED_VAR(lockedPtr) =                      \
           (RDD_ARG_2_OR_1(__VA_ARGS__)).operator->();           \
         !SYNCHRONIZED_VAR(state);                               \
         SYNCHRONIZED_VAR(state) = true)                         \
      for (auto& RDD_ARG_1(__VA_ARGS__) =                        \
...
#define TIMED_SYNCHRONIZED(timeout, ...)                         \
  if (bool SYNCHRONIZED_VAR(state) = false) {                    \
    ...
    for (auto SYNCHRONIZED_VAR(lockedPtr) =                      \
           (RDD_ARG_2_OR_1(__VA_ARGS__)).timedAcquire(timeout);  \
         !SYNCHRONIZED_VAR(state);                               \
         SYNCHRONIZED_VAR(state) = true)                         \
      for (auto RDD_ARG_1(__VA_ARGS__) =                         \
...
#define SYNCHRONIZED_CONST(...)                \
  SYNCHRONIZED(                                \
      RDD_ARG_1(__VA_ARGS__),                  \
      (RDD_ARG_2_OR_1(__VA_ARGS__)).asConst())
