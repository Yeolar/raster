...
/// that don't use noexcept but that are marked rdd::IsRelocatable
/// and std::is_nothrow_constructible, which is common for rdd types.
/// In particular, if you can declare RDD_ASSUME_FBVECTOR_COMPATIBLE
...
class MPMCQueue {
 ...
 protected:
  ...
  size_t RDD_ALIGN_TO_AVOID_FALSE_SHARING capacity_;
  ...
  std::atomic<uint64_t> RDD_ALIGN_TO_AVOID_FALSE_SHARING pushTicket_;

  /// Dequeuers get tickets from here
  std::atomic<uint64_t> RDD_ALIGN_TO_AVOID_FALSE_SHARING popTicket_;
  ...
  std::atomic<uint32_t> RDD_ALIGN_TO_AVOID_FALSE_SHARING pushSpinCutoff_;

  /// The adaptive spin cutoff when the queue is empty on dequeue
  std::atomic<uint32_t> RDD_ALIGN_TO_AVOID_FALSE_SHARING popSpinCutoff_;
...
struct SingleElementQueue {
  ...
  void dequeue(uint32_t turn,
    ...
    dequeueImpl(turn,
                ...
                updateSpinCutoff,
                elem,
                typename std::conditional<rdd::IsRelocatable<T>::value,
                                          ImplByRelocation,
                                          ImplByMove>::type());
  }
