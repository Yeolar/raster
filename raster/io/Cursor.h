...
class CursorBase {
 ...
 public:
  ...
  void clone(std::unique_ptr<rdd::IOBuf>& buf, size_t len) {
    if (UNLIKELY(cloneAtMost(buf, len) != len)) {
      std::__throw_out_of_range("underflow");
    }
  }

  void clone(rdd::IOBuf& buf, size_t len) {
    if (UNLIKELY(cloneAtMost(buf, len) != len)) {
      std::__throw_out_of_range("underflow");
    }
  }

  size_t cloneAtMost(rdd::IOBuf& buf, size_t len) {
    std::unique_ptr<rdd::IOBuf> tmp;
    size_t copied = 0;
  ...
  size_t cloneAtMost(std::unique_ptr<rdd::IOBuf>& buf, size_t len) {
    if (!buf) {
      buf = make_unique<rdd::IOBuf>();
    }
...
class Appender : public detail::Writable<Appender> {
 public:
   ...
   * format specifier.
   *
   * Note that rdd/Format.h provides nicer and more type-safe mechanisms
   ...
   */
  void printf(const char* fmt, ...) RDD_PRINTF_FORMAT_ATTR(2, 3);
...
class QueueAppender : public detail::Writable<QueueAppender> {
 public:
  ...
  void insert(const rdd::IOBuf& buf) {
    insert(buf.clone());
  }

 private:
  rdd::IOBufQueue* queue_;
  size_t growth_;
};
...
#include "raster/io/Cursor-inl.h"
