...
class IOBufQueue {
 public:
   ...
   */
  void append(std::unique_ptr<rdd::IOBuf>&& buf, bool pack=false);
   ...
   *         in the queue.
   */
  std::unique_ptr<rdd::IOBuf> split(size_t n) {
    return split(n, true);
  }
   ...
   */
  std::unique_ptr<rdd::IOBuf> splitAtMost(size_t n) {
   ...
   */
  std::unique_ptr<rdd::IOBuf> pop_front();
 ...
 private:
  ...
  std::unique_ptr<rdd::IOBuf> split(size_t n, bool throwOnUnderflow);
  ...
  std::unique_ptr<rdd::IOBuf> head_;
};
