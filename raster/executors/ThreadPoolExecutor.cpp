...
#include "raster/executors/ThreadPoolExecutor.h"
...
void ThreadPoolExecutor::runTask(const ThreadPtr& thread, Task&& task) {
  thread->idle = false;
  auto startTime = acc::timestampNow();
  ...
  if (task.expiration_ > 0 && task.stats_.waitTime >= task.expiration_) {
    ...
    task.stats_.runTime = acc::timestampNow() - startTime;
  }
  thread->idle = true;
  thread->lastActiveTime = acc::timestampNow();
...
ThreadPoolExecutor::PoolStats ThreadPoolExecutor::getPoolStats() {
  const auto now = acc::timestampNow();
  acc::RWSpinLock::ReadHolder r{&threadListLock_};
